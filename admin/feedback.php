    ...
    <section class="content">
        <div class="box">
            <div class="box-header">
                <h1 class="box-title">Product review</h1>
            </div>
            <!-- /.box-header -->
            <div class="box-body table-responsive">
              ...
              ';
              $sl = 1;
                $sql_product_review = "SELECT * from product_review order by id desc";
                $query_product_review = mysqli_query($con,$sql_product_review);
                if (mysqli_num_rows($query_product_review) > 0) {
                  while($row_product_review = mysqli_fetch_array($query_product_review)) {
                    $row_product = mysqli_fetch_array(mysqli_query($con,"SELECT * from products where id = '".$row_product_review['product_id']."' limit 1 "));
                    echo '
                    <tr>
                        <td>
                        '
                        .$sl.
                        '</td><td><a href="../product-details?product_id='
                        .$row_product['id'].
                        '">'.$row_product['product_name'].'</a></td><td>'
                        .$row_product_review['review'].
                        '</td><td>'
                        .date('F d, Y', strtotime($row_product_review['created_at'])).
                        '</td><td class="text-center"><a class="text-red" href="?review_id='
                        .$row_product_review['id'].
